...
public class Tetrahedral extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        Cylinder bond2 = createConnection(new Point3D(0, 0, 0),
                new Point3D(CYLHEIGHT/Math.sqrt(2), CYLHEIGHT/Math.sqrt(2), CYLHEIGHT/Math.sqrt(2)));


        Cylinder bond3 = createConnection(new Point3D(0, 0, 0),
                new Point3D(CYLHEIGHT/Math.sqrt(2), CYLHEIGHT/Math.sqrt(2), -CYLHEIGHT/Math.sqrt(2)));
        bond3.setMaterial(outerAtomMaterial);

        Cylinder bond4 = createConnection(new Point3D(0, 0, 0),
                new Point3D(-CYLHEIGHT/Math.sqrt(2), CYLHEIGHT/Math.sqrt(2), -CYLHEIGHT/Math.sqrt(2)));

        bond3.setMaterial(centralAtomMaterial);

        Group group = new Group( centralAtom, outerAtom1, bond1, bond2, bond3, bond4);
